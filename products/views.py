...
def all_products(request):
    """ A view to show all products including sorting and searcxheing quieries """
    ...
    if request.GET:
        ...
        if 'q' in request.GET:
            query =  request.GET['q']
            if not query:
                messages.error(request, "You didn't entre any search criteria")
                return redirect(reverse('products'))
            quieries = Q(name__icontains=query) | Q(description__icontains=query)
...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            product = form.save()
            messages.success(request, 'Succcessfully addded product!')
